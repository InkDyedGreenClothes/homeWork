...
export default class Dialog extends EventTarget {
    constructor(options) {
        ...
        // 根据for in 遍历去进行配置对象合并
        this.opts = this.myExtend({}, defalultOptions, options);

        // 使用es6 展开运算符进行对象合并
        // this.opts = {...defalultOptions, ...options}
        
        // console.log(this.opts);
        // console.log('defalultOptions');
        // console.log(defalultOptions);

        this.init();
    }
    // 遍历赋值  将配置对象合并
    myExtend(callbackObj, defaultObj, newObj) {
        // 遍历默认配置 
        for (let key in defaultObj) {
            // 判断传入配置中是否存在 key  如果存在就将新值赋给 默认配置
            if (newObj.hasOwnProperty(key)) {
                callbackObj[key] = newObj[key];
            } else {
                callbackObj[key] = defaultObj[key];
            }
        }
        return callbackObj;
    }
